...
namespace Pages
{
    public partial class MainWindow : Window, IDisposable
    {
        ...
        void MarqueeTimer_Tick(object sender, EventArgs e)
        {
            ...
            {
                count = count + 10;
                marScroll.ScrollToHorizontalOffset(count);
            }
            else
            {
                count = count - 10;
        ...
        private void AddResultsToGrid(List<Result> ResultList) {
            grdTweets.ColumnDefinitions.Clear();
            grdTweets.Children.Clear();

            foreach (Result Status in ResultList) {

                // Create the profile image for grid
                grdTweets.ColumnDefinitions.Add(new ColumnDefinition() { Width = new GridLength(50) });
                ...
                Grid.SetColumn(ProfileImage, grdTweets.ColumnDefinitions.Count - 1);
                grdTweets.Children.Add(ProfileImage);

                grdTweets.ColumnDefinitions.Add(new ColumnDefinition() { Width = new GridLength(300) });
                ...
                Grid.SetColumn(TextBlock, grdTweets.ColumnDefinitions.Count -1);
